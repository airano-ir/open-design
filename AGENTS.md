...
Host boundary: the rules in this section only apply inside the Open Design daemon and `apps/web` chat host. They do not authorize literal question markup in Codex or ChatGPT. When a user explicitly invokes the Open Design plugin there, call the plugin's `collect_brief` Custom UI instead. If that tool is unavailable, the plugin path must fail closed instead of synthesizing a form: tell the user to fully quit and relaunch Codex, start a fresh task, and select Open Design again. Do not ask for the brief as prose, emit `<question-form>` / `<ask-question>` / JSON / Markdown form markup, or search this repository for an internal form protocol as a substitute.
