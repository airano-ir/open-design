...
test("[P2] captures the home plugin filtered surface", async ({ page }) => {
  ...
  await expect(
    home.locator(
      'article.plugins-home__card[data-plugin-id="visual-deck-writer"]'
    )
  ).toBeVisible();
...
test("[P2] captures the home plugin detail surface", async ({ page }) => {
  ...
  await card.hover();
  await home
    .getByTestId("plugins-home-details-visual-deck-writer")
    .click({ force: true });
  await expect(
    page.getByRole("dialog", { name: /Deck Writer preview/i })
  ).toBeVisible();
  await expect(
    page.getByTestId("plugin-details-use-visual-deck-writer")
  ).toBeVisible();
  await expect(
    page.locator(".ds-modal-stage-iframe-scaler iframe")
  ).toBeVisible();
...
test("[P2] captures the home context picker surface", async ({ page }) => {
  ...
  await expect(
    page.getByRole("option", { name: /Prototype Starter/i })
  ).toBeVisible();
...
test("[P2] captures the projects page surface", async ({ page }) => {
  ...
  await expect(
    projects.getByRole("heading", { name: "Projects" })
  ).toBeVisible();
...
test("[P2] captures the projects kanban surface", async ({ page }) => {
  ...
  await expect(projects.getByTestId("designs-view-kanban")).toHaveAttribute(
    "aria-pressed",
    "true"
  );
  await expect(projects.getByText("Launchpad dashboard").first()).toBeVisible();
...
test("[P2] captures the plugins page surface", async ({ page }) => {
  ...
  await expect(
    plugins.getByRole("heading", { name: "Plugins", exact: true })
  ).toBeVisible();
...
test("[P2] captures the integrations use everywhere surface", async ({
  page,
}) => {
  await configureVisualPage(page);
  await gotoVisualHome(page);
  ...
  await expect(
    page.getByTestId("integrations-tab-use-everywhere")
...
test("[P2] captures the topbar execution switcher surface", async ({
  page,
}) => {
  await configureVisualPage(page);
  await gotoVisualHome(page);
  ...
  await expect(
    page.getByTestId("inline-model-switcher-mode-daemon")
  ).toBeVisible();
...
test("[P2] captures the settings execution surface", async ({ page }) => {
  ...
  await expect(
    dialog.getByRole("tablist", { name: "Execution mode" })
  ).toBeVisible();
  await waitForVisualFonts(page);
...
test("[P2] captures the settings BYOK surface", async ({ page }) => {
  ...
  await expect(
    dialog.getByRole("tablist", { name: "API protocol" })
  ).toBeVisible();
  await expect(
    dialog.getByRole("heading", { name: "Anthropic API" })
  ).toBeVisible();
  await waitForVisualFonts(page);
...
async function openSettingsDialog(
  page: Parameters<typeof configureVisualPage>[0]
) {
...
async function gotoVisualWorkspace(
  page: Parameters<typeof configureVisualPage>[0]
) {
  await page
